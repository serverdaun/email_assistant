...
from src.schemas import RouterSchema, State, StateInput, UserPreferences
from src.tools.base_tools import get_tools, get_tools_by_name
from src.tools.gmail_tools import mark_as_read
from src.utils import format_for_display, format_gmail_markdown, parse_gmail
...
llm_router = init_chat_model("openai:gpt-4.1", temperature=0.0).with_structured_output(
    RouterSchema
)

# Initialize the LLM, enforcing tool use (of any available tools) for agent
llm_with_tools = init_chat_model("openai:gpt-4.1", temperature=0.0).bind_tools(
...
def llm_call(state: State, store: BaseStore):
    ...
    cal_preferences = get_memory(
        store, ("email_assistant", "cal_preferences"), DEFAULT_CALENDAR_PREFERENCES
    )
