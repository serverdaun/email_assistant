...
def get_credentials(gmail_token=None, gmail_secret=None):
    ...
    if gmail_token:
        try:
            token_data = (
                json.loads(gmail_token) if isinstance(gmail_token, str) else gmail_token
            )
    ...
    # Return None if we couldn't get token data from any source
    if token_data is None:
    ...
    try:
        ...
        for date_str in dates:
            ...
            if "all-day" in [slot[0] for slot in busy_slots]:
                ...
                work_end = datetime(
                    year=int(year), month=int(month), day=int(day), hour=18, minute=0
                )
...
def schedule_meeting_tool(
    ...
    Args:
        ...
        end_time: Meeting end time in ISO format (YYYY-MM-DDTHH:MM:SS)
        organizer_email: Email address of the meeting organizer
        timezone: Timezone for the meeting (default: Europe/Prague)
